...
$(function(){
	if($.cookie("back") == null){
		//window.alert($.cookie('comment_seg_modify'));
		if($.cookie('comment_seg_modify') != null)
			document.getElementById("comment").innerText = $.cookie('comment_seg_modify');
		$.cookie("comment", null);
		sent = $.cookie('modify');
		...
		//window.alert($.cookie('comment_seg_modify') + "back"+ $.cookie("back") );
		if($.cookie('comment_seg_modify') != null)
			document.getElementById("comment").innerText = $.cookie('comment_seg_modify');
		sent = $.cookie('segsent');
		psent =$.cookie('psent');
	...
	$("#save").click(function(){
		...
		$.cookie('comment_seg_modify', comment);
		//window.alert($.cookie('user') + " %%`$$ " + psent + " %%`$$ " + segsent + "%%`$$" + comment);
		...
		if(segsent2 != psent){//改变了原始句子
			...
			url = "writeseg_modify.php";
			$.post(
				url, 
				{psent: psent, segsent: segsent, comment: comment, fsent:fsent},
				function (data){
					//window.alert(data);//接下来将segsent和tags重新组织，递交给tagging页面
					$.cookie('segsent', fsent);
					$.cookie('psent', psent);
					$.cookie('restart', null);
					window.location.href="POSTagging_modify.php";
				}
			);
		}
	});
				...
				if(t < start){
					...
					if(start >0){//window.alert("start" + sent[start-1]);
						if(sent[start] != ' ') {
							...
							}else if(sent[start-1] != ' '){
								sent2 += "  ";//加了2个空格
							}
						}else if(sent[start] == ' '){
							if(sent[start-1] != ' ' && sent[start+1] != ' '){
								sent2 += " ";//加了1个空格
							}
						}
					}
...
<div class="header">
	...
	<ul class="userinfo" >
        <li id="headuser">
			...
			?>
			<a href="historyc.php" id="history">历史记录</a> | <span class='download' id='questions'>常见问题</span> | <a href="#" id="logout">退出</a> | <a id='login' href='loginc.php'>返回</a>
        </li>
    </ul>
</div>
