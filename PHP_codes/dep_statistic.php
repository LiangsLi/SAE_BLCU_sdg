...
$(function(){
	...
	$("#fresh").click(function(){
		window.location.href="dep_statistic.php";
	});
});
...
$(function(){
	$("#name").keydown(function(e){
		if(e.keyCode==13){
			...
			$.post(
				"get_depstatistic.php", 
				{annoter:val},
				function (data){
					window.alert(data);//原句%%`$$句子%%`$$时间%%`$$chargecomment原句%%`$$句子%%`$$时间%%`$$chargecomment
					var tmp = data.split("%%`$$");
					
					var str = "已完成标注数目：" + tmp[0]+"<br>准确率为：" + tmp[1] +"%<br><br>" + "错误内容为:<br>";
					document.getElementById('result').innerHTML = str;
				if(tmp.length>2){
					Str2 = data.split("%%`$$");
					recordlen = Str2.length-3;//数组的结尾有一个空值
					var listnum = 1;
					...
					for(var j = 2; j< recordlen; j+= 4){//显示
						var sent_0 = Str2[j];
						var res_sent_1 = Str2[j+1];
						var res_time = Str2[j+2];
						var chargecomment = Str2[j+3];
						...
						var table = "<table id='content"+listnum+"' class='words' width='"+(maxwords*40 + 190)+"' style='table-layout: fixed;word-break:break-all' cellpadding=0 cellspacing=0 border=1 >";

						if(arcnum.length >=2){
							var canv ="<tr><th width = 50 rowspan='3'>"+ listnum+ "</th><td width = 50 rowspan='3'>"+ chargecomment +"</td><td rowspan='3' width=90>"+ res_time+ "</td><td colspan='"+ maxwords +"'><canvas id='cc" + listnum +"' class='canvas' width='"+(40 * maxwords)+"' height='"+height+"' style='border:0px solid #888;' > Your browser dosen't support the HTML5 canvas.</canvas></td></tr>";
							
							var tmptable = document.getElementById('table').innerHTML;
							tmptable += table + canv+ wordlist + taglist + "</table>";
						}else{
							var canv ="<tr><th width = 50 >"+ listnum+ "</th><td width = 50>"+ chargecomment +"</td><td width=90>"+ res_time+ "</td><td >"+ skipsent +"</td></tr>";
					...
					listnum = 1;//
					for(var j = 2; j< recordlen; j += 4){//开始画弧
						if(jflag[j] == 1){
							listnum += 1;
							continue;
						}
...
<body oncontextmenu=self.event.returnValue=false>
	...
	<form name="form2" id="form2" action="javascript:checkSumbit(); ">
		...
		<button type="button"class="save" id="fresh">刷新</button>
	</form>
	</div>
