...
$(function(){
	if($.cookie('comment_tag_modify') != null){
		document.getElementById("comment").innerText = $.cookie('comment_tag_modify');
	}
	...
	$("#save").click(function(){
		...
		if(sentpp != psent){//词性标注的句子和原来句子不一样了
			//winidow.alert("句子内容被改变，请重做！");
			comment += "|句子内容被改变";
		}

		var url = "writetags_modify.php";
		$.post(
			url, 
	...
	$("#back").click(function(){  
		$.cookie('back', 1);
		var comment = document.getElementById("comment").value;
		$.cookie('comment_tag_modify', comment);
...
<button id="back" type="button" class="save">返回</button>
