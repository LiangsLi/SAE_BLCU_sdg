...
$(function(){
	...
	$("#submit").click(function(){
		...
		$.post(
			"login.php", //链接login.php文件（负责检查用户输入的用户名和密码是否正确，同时更新登录次数。）
			...
			function (data){
				...
				if(data[0]=='1'){//用==判断不了，郁闷！！！！
					...
					$.cookie("sentnum", null);
					window.location.href="label_dep.php";//在当前页面打开label_dep.php
				}
			},
			"text"
		);
...
<div class="outside">
	...
	<div class="content">
		...
		<form id="login" name="login" >
				...
				<div class="item">
					<label>&nbsp;</label>
					<input type="button" value="登 录" name="submit" id="submit" class="login-submit" tabindex="5" />
				</div>
		</form>
		</div>
	</div>	 
