...
if (isset($_COOKIE["user"])){//此处为页面第一次获取'finish'cookie
	//echo $_COOKIE["user"];
	//$sql2 = "select complete from signup where username='". $_COOKIE["user"] ."'";//求完成的标注句子数目
	$sql2 = "select count(*) as complete from fillout where annoter='". $_COOKIE["user"] ."' and tagsent != '';";
	mysql_query("SET NAMES 'UTF8'");
	$res22 = mysql_query($sql2);
	while($times2 = mysql_fetch_assoc($res22)){
		$firstcomplete = $times2['complete'];
		setcookie("finish", $times2['complete']);
	}
}else{
	$url = "index.php"; 
...
if($_COOKIE['sentnum'] == null){
	...
	$sql = "select asbtsent , sent from fillout where annoter='". $username ."' and segsent != '' and segtime!= '' and tagsent='' and tagtime = '';";
	mysql_query("SET NAMES 'UTF8'");
	...
	while($times = mysql_fetch_assoc($res2)){
		$sents .= $times['asbtsent'] ."%%`$$";
		$psent .= $times['sent'] ."%%`$$";
		$num += 1;
	}
	
	echo "<form><input type='hidden' id='hiddenum' class='hidde' value='".$num ."'></input></form>";//这些句待补充完整的句子
	echo "<form><input type='hidden' id='hidde' class='hidde' value='".$sents."' /></form>";
	echo "<form><input type='hidden' id='hiddepsent' class='hidde' value='".$psent."' /></form>";
	//setcookie("sentstr", $sents);这句此处正确，但是在js中取这个cookie时，' '->'+'.郁闷！！！！
	if($num == 0){
		header("Location: http://blcusdg.applinzi.com/WSegmentation.php");
	}
}
?>
...
$(function(){
	...
	var line2 = 1;
	var col = 0;
	//window.alert(document.getElementById("hidde").value + "   " + document.getElementById("hiddepsent").value);
	//window.alert(document.getElementById("hiddenum").value);

	var sentnum = Number(document.getElementById("hiddenum").value)-1;
	document.getElementById("sentnum").innerHTML = "<b>" + (sentnum+1) + "</b>";
	//window.alert("sentstr" + $.cookie("sentstr"));//sent获取,这句' '->'+'!!!!!!!!莫名错误
	
	sentword = document.getElementById("hidde").value.split("%%`$$");
	psentw = document.getElementById("hiddepsent").value.split("%%`$$");
	sent = sentword[sentnum];
	psent = psentw[sentnum];
	//window.alert(sentnum + "sent:" + sent + "psent:" + psent);
	...
	$("#save").click(function(){
		...
		$.post(
			url, 
			{psent: psent, fsent: fsent, comment: comment},//完成一个句子，完成数目加1
			function (data){
				//写入tagsent成功//完成一个句子，更新页面显示
				document.getElementById("hiddenum").value = sentnum;
				...
				if((sentnum -1) >=0)
					window.location.href = "Tagging_unfi.php";
				else
...
function change(tag){
	if(tt.rows.item(line).cells.item(col).innerText != ""){
		tt.rows.item(line).cells.item(col).innerText = tag;
	}
	tt.rows.item(line).cells.item(col).style.background="#eee";
	//var rows = tt.rows.length ; //表格行数 //var cells = tt.rows.item(0).cells.length ; //表格列数     
}
...
<div class="header">
	<p class="logo" >
        您已完成<label class='finish' id="finish"><?php echo " <b>".$firstcomplete."</b> "; ?></label>句标注任务
    </p>
	<ul class="userinfo" >
        <li id="headuser">
			<?php
			    ...
			    if($users != ""){
					echo $users ." | <span id='questions' class='download'>常见问题</span>";
				}else{
...
<div></div>
<p>&nbsp;</p>
<p class="words">消息提示：您有 <label id="sentnum"> </label> 个句子尚未完成词性标注！</p>
