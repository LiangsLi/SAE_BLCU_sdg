...
$(function(){
	$("#pos").click(function(){
		//window.alert("分词任务已完成！");
		window.location.href="Tagging_unfi.php";
	});

	$("#dep").click(function(){
		/*if($.cookie('user') == '丁宇' || $.cookie('user') == '曲咏措姆' || $.cookie('user') == 'Car' || $.cookie('user') == 'zhenghuali' || $.cookie('user') == 'jguo')
		if($.cookie('user') == '丁宇')
			window.location.href="label_dep.php";
		else
			//window.alert('该页面尚未启动');*/
		//window.alert("导入数据，系统暂停下！");
		window.location.href="label_dep.php";
	});
...
<body class="bg_body">
	<div class="header">
		...
		<ul class="userinfo">
			<li>
			<?php $user = $_COOKIE['user'];echo $user;?> | <a href="#" id="logout">退出</a>
			</li>
		</ul>
	</div>

	<div class='outside'>	
		<button id='pos' class='save' type='button'>进入分词、词性标注页面</button>
		<button id='dep' class='save' type='button'>进入依存句法标注页面</button>	
	</div>
</body>
</html>